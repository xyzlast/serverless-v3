...
// `require()` of a native ES module (supported since Node.js v20.19/v22.12) resolves with a module
// namespace object, where the configuration is exposed at its `default` export. Unwrap it, so such
// configuration is read the same way as it is by the runtimes which cannot `require()` ES modules
const resolveModuleValue = (mod) => {
  if (mod && mod[Symbol.toStringTag] === 'Module' && 'default' in mod) return mod.default;
  return mod;
};

const requireConfiguration = async (configurationPath) => {
  try {
    return require(configurationPath);
  } catch (error) {
    // Older runtimes reject `require()` of ES modules outright, while newer ones do so only for
    // the modules relying on top-level await. Fall back to `import()` in both cases
    if (error.code === 'ERR_REQUIRE_ESM' || error.code === 'ERR_REQUIRE_ASYNC_MODULE') {
      return require('../utils/import-esm')(configurationPath);
    }
    throw error;
  }
};
...
const parseConfigurationFile = async (configurationPath) => {
  switch (path.extname(configurationPath)) {
    ...
    case '.js': {
      ...
      try {
        return resolveModuleValue(await requireConfiguration(configurationPath));
      } catch (error) {
        throw new ServerlessError(
